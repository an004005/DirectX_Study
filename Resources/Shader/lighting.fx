...
PS_OUT PS_DirLight(VS_OUT input)
{
	...
	if (viewPos.z <= 0.f) // view space에서 camera뒤에 있는 물체는 z가 음수
		clip(-1); // -1을 주면, 중간에 끝나게됨(내장함수)
...
// g_tex_0 : Position RT
// g_tex_1 : Normal RT
// g_vec2_0 : RenderTarget Resolution(해상도, 800 * 600)
// Mesh : Sphere
...
PS_OUT PS_PointLight(VS_OUT input)
{
	...
	// input.pos = SV_Position = Screen 좌표: pixel좌표계 즉 x는 0~800사이 // deferred rendering 27분
	float2 uv = float2(input.pos.x / g_vec2_0.x, input.pos.y / g_vec2_0.y);
	// uv 좌표는 왼위가 0으로 최대 1이므로 위 계산 성립
